...
class AmlPipelineIdempotencyTest extends AbstractIntegrationTest {
  ...
  @Test
  @DisplayName(
      "Should process duplicate Kafka events idempotently without duplicating amounts in Cassandra")
  void shouldHandleDuplicateKafkaMessagesIdempotently() throws JsonProcessingException {
    ...
    List<RuleResult> ruleResults =
        List.of(
            new RuleResult(
                "RULE_MAX_AMOUNT", 1, RuleStatus.PASSED, "Amount is within limit", 12L, false),
        ...
        .untilAsserted(
            () -> {
              ...
              BigDecimal actualSum =
                  cassandraHistoryRepository.sumAmount(
                      accountFrom,
                      now.minus(Duration.ofMinutes(1)),
                      now.plus(Duration.ofMinutes(1)));

              assertThat(actualSum).isNotNull().isEqualByComparingTo(amount);
            });
  }
}
