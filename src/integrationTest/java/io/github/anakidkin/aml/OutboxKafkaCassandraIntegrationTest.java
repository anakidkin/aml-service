...
class OutboxKafkaCassandraIntegrationTest extends AbstractIntegrationTest {
  ...
  void shouldProcessFullOutboxToCassandraPipeline() {
    ...
    var outboxEvent =
        jpaOutboxRepository.findAll().stream()
            ...
            () -> {
              ...
              BigDecimal actualSum =
                  cassandraHistoryRepository.sumAmount(
                      accountFrom, testStartTime, Instant.now().plus(Duration.ofMinutes(1)));
              assertThat(actualSum).isNotNull().isEqualByComparingTo(expectedAmount);
            });
  }
}
